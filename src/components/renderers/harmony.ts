...
export function renderFibonacciOverlay(
  ...
  fibs.forEach((fib, i) => {
    ...
    const rect = new paper.Path.Rectangle(new paper.Point(x, y), new paper.Point(x + w, y + h));
    const rectColor = hexToColor(style.color, style.opacity * (0.3 + i * 0.08));
    showIfIntersects(rect, context, () => {
      rect.strokeColor = rectColor; rect.strokeWidth = style.strokeWidth;
      rect.fillColor = hexToColor(style.color, style.opacity * 0.03);
      if (w > 12) {
        const label = new paper.PointText(new paper.Point(x + w / 2, y + h / 2 + 3));
        label.content = String(fib); label.fillColor = labelColor;
        label.fontSize = Math.min(9, w * 0.4); label.justification = 'center';
      }
    });
    x += w;
  });
